...
let rec is_dice_clicked img_dim game =
  let x = game.game_screen.dice.x_coord in
  let y = game.game_screen.dice.x_coord in
  if button_down () then
    if dimension_check (mouse_pos ()) img_dim x y then
      Functionality.handle2 0
    else is_dice_clicked img_dim game
  else is_dice_clicked img_dim game

let draw_dice_roll img_dim game =
  let x = game.game_screen.dice.x_coord + 50 in
  let y = game.game_screen.dice.y_coord + 50 in
  let z = is_dice_clicked img_dim game in
  let img = string_of_int z ^ ".png" in
  draw_img img x y
